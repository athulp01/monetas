import { TRANSACTION_TYPE, type PrismaClient } from "@prisma/client";
...
export const getTotalExpensesForTheMonth = (
  month: Date,
  client: PrismaClient,
) => {
  ...
  return client.transaction.aggregate({
    ...
    where: {
      AND: [
        {
          timeCreated: {
            gt: momentDate.startOf("month").toISOString(),
          },
        },
        {
          timeCreated: {
...
export const getTotalIncomeForTheMonth = (
  month: Date,
  client: PrismaClient,
) => {
  const momentDate = moment(month);
...
export const getNetIncomePerDay = (
  rangeStart: Date,
  rangeEnd: Date,
  client: PrismaClient,
) => {
...
export const getNetExpensePerPayee = (
  rangeStart: Date,
  rangeEnd: Date,
  client: PrismaClient,
) => {
