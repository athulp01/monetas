...
import { ControlledDateTime } from '~/components/forms/ControlledDateTime'
import { CurrencyFormatter, DateFormater } from '~/lib/utils'
import { INVESTMENT_TYPE } from '@prisma/client'

export type InvestmentList = RouterOutputs['investment']['listInvestments']['investments']
...
const InvestmentsTableView = (props: Props) => {
  const [isInEditMode, setIsInEditMode, createForm, editForm] = useTable<InvestmentList[0]>()
          ...
          <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            <thead className="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <TableHeader title="Buy Date" isSortable></TableHeader>
                ...
                <tr className="border-b bg-white dark:border-gray-700 dark:bg-gray-800 ">
                  ...
                  <td className="px-1 py-4">
                    <div className="flex w-40">
                      ...
                      <input
                        form="createForm"
                        {...createForm.register('currentPrice', { required: true })}
                ...
                >
                  ...
                  <td className="px-1 py-4">
                    {isInEditMode === i ? (
                      ...
                      DateFormater.format(investment.buyDate)
                    )}
                  </td>
                  <td className="px-1 py-4">
                    {isInEditMode === i ? (
                      ...
                      <NumberDynamic value={Math.abs(investment?.currentPrice)}></NumberDynamic>
                    )}
                  </td>
                  <td className="px-1 py-4">
                    {isInEditMode === i ? (
                      ...
                      >
                        ...
                        {CurrencyFormatter.format(
                          (investment?.currentPrice - investment?.buyPrice) * investment?.units
                        ) +
                          ` (${
                            investment?.currentPrice - investment?.buyPrice < 0 ? '' : '+'
                          }${Math.floor(
                            ((investment?.currentPrice - investment?.buyPrice) * 100) /
                              investment?.buyPrice
                          )}%)`}
                      </span>
                    )}
